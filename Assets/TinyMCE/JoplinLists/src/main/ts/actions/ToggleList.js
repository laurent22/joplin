...
const updateList = function (editor, list, listName, detail) {
    if (list.nodeName !== listName) {
        ...
        (0, Events_1.fireListEvent)(editor, (0, ListAction_1.listToggleActionFromListName)(listName), newList);
    }
    else {
        updateListWithDetails(editor.dom, list, detail);
        (0, Events_1.fireListEvent)(editor, (0, ListAction_1.listToggleActionFromListName)(listName), list);
    }
};
...
const toggleSingleList = function (editor, parentList, listName, detail) {
    ...
    if (parentList) {
        const listType = (0, JoplinListUtil_1.findContainerListTypeFromElement)(parentList);
        if (parentList.nodeName === listName && !hasListStyleDetail(detail) && !(0, Util_1.isCustomList)(parentList) && listType === detail.listType) {
            (0, Indendation_1.flattenListSelection)(editor);
        }
        else {
            const bookmark = Bookmark.createBookmark(editor.selection.getRng(true));
        ...
        (0, Events_1.fireListEvent)(editor, (0, ListAction_1.listToggleActionFromListName)(listName), parentList);
    }
};
