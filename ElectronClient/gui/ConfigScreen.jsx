...
class ConfigScreenComponent extends React.Component {
	constructor() {
		...
		this.state.shouldUseDarkColors = bridge().shouldUseDarkColors();
	...
	sectionToComponent(key, section, settings, selected) {
		...
		if (section.name === 'appearance') {
			if (settings['themeAutoDetect']) {
				// Append preferred theme options below Auto Detect theme checkbox
				settingComps.splice(1, 0, this.settingToComponent('preferredLightTheme', settings['preferredLightTheme']));
				settingComps.splice(2, 0, this.settingToComponent('preferredDarkTheme', settings['preferredDarkTheme']));
			} else {
				settingComps.splice(1, 0, this.settingToComponent('theme', settings['theme']));
			}
		}
	...
	settingToComponent(key, value) {
		...
		const detectTheme = (autoDetect) => {
			this.setState({ shouldUseDarkColors: bridge().shouldUseDarkColors() });

			let changeSetting = '';
			// active theme should change to preferred theme based on system detected theme
			if (autoDetect) {
				this.state.shouldUseDarkColors ? changeSetting = 'preferredDarkTheme' : changeSetting = 'preferredLightTheme';
			} else {
				changeSetting = 'theme';
			}

			updateSettingValue('theme', settings[changeSetting]);
			Setting.setValue('theme', settings[changeSetting]);
		};
				...
				<div key={key} style={rowStyle}>
					...
					<select
						...
						onChange={event => {
							...
							// if Auto Detect option is ON theme should change according to preferred theme options
							if (settings['themeAutoDetect']) {
								// preferred dark theme should only work for dark system theme
								if (key == 'preferredDarkTheme' && this.state.shouldUseDarkColors) {
									updateSettingValue('theme', event.target.value);
								} else if (key == 'preferredLightTheme' && !this.state.shouldUseDarkColors) {
									updateSettingValue('theme', event.target.value);
								}
							}
						}}
