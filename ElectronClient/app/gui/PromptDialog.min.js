...
class PromptDialog extends React.Component {
	...
	render() {
		...
		if (this.props.inputType === 'datetime') {
			inputComp = React.createElement(Datetime, { value: this.state.answer, inputProps: { style: styles.input }, dateFormat: time.dateFormat(), timeFormat: time.timeFormat(), onChange: momentObject => onDateTimeChange(momentObject) });
		} else if (this.props.inputType === 'tags') {
			inputComp = React.createElement(CreatableSelect, { styles: styles.select, theme: styles.selectTheme, ref: this.answerInput_, value: this.state.answer, placeholder: '', components: makeAnimated(), isMulti: true, isClearable: false, backspaceRemovesValue: true, options: this.props.autocomplete, onChange: onSelectChange, onKeyDown: event => onKeyDown(event) });
		} else if (this.props.inputType === 'dropdown') {
			inputComp = React.createElement(Select, { styles: styles.select, theme: styles.selectTheme, ref: this.answerInput_, components: makeAnimated(), value: this.props.answer, defaultValue: this.props.defaultValue, isClearable: false, options: this.props.autocomplete, onChange: onSelectChange, onKeyDown: event => onKeyDown(event) });
		} else {
		...
		const buttonComps = [];
		if (buttonTypes.indexOf('ok') >= 0) buttonComps.push(React.createElement(
			'button',
			{ key: 'ok', style: styles.button, onClick: () => onClose(true, 'ok') },
			_('OK')
		));
		if (buttonTypes.indexOf('cancel') >= 0) buttonComps.push(React.createElement(
			'button',
			{ key: 'cancel', style: styles.button, onClick: () => onClose(false, 'cancel') },
			_('Cancel')
		));
		if (buttonTypes.indexOf('clear') >= 0) buttonComps.push(React.createElement(
			'button',
			{ key: 'clear', style: styles.button, onClick: () => onClose(false, 'clear') },
			_('Clear')
		));
		...
		return React.createElement(
			...
			React.createElement(
				...
				React.createElement(
					'div',
					{ style: { display: 'inline-block', color: 'black', backgroundColor: theme.backgroundColor } },
					inputComp,
					descComp
				),
				React.createElement(
