...
class CustomHighlightRules extends ace.acequire('ace/mode/markdown_highlight_rules').MarkdownHighlightRules {
	constructor() {
		super();
...
class NoteTextComponent extends React.Component {
			...
			if (!ranges || !ranges.length || !this.state.note) {
				...
				if (process.platform === 'linux') {
					...
					if (textRange.start != textRange.end) {
						clipboard.writeText(this.state.note.body.slice(Math.min(textRange.start, textRange.end), Math.max(textRange.end, textRange.start)), 'selection');
					}
				}
			}
		};
	...
	async commandSavePdf(args) {
		try {
			...
			for (let i = 0; i < noteIds.length; i++) {
				...
				const pdfPath = noteIds.length === 1 ? path : await shim.fsDriver().findUniqueFilename(`${path}/${this.pdfFileName_(note, folder)}`);
	...
	renderMultiNotes(rootStyle) {
		...
		for (let i = 0; i < menuItems.length; i++) {
			...
			itemComps.push(React.createElement(
				'button',
				{ key: item.label, style: buttonStyle, onClick: () => multiNotesButton_click(item) },
				item.label
			));
		}
	...
	render() {
		...
		if (this.state.showRevisions && note && note.id) {
			...
			return React.createElement(
				'div',
				{ style: rootStyle },
				React.createElement(NoteRevisionViewer, { noteId: note.id, customCss: this.props.customCss, onBack: this.noteRevisionViewer_onBack })
			);
		}
		...
		if (this.canDisplayTagBar()) {
			...
			toolbarStyle.marginBottom = tagStyle.marginBottom, bottomRowHeight = rootStyle.height - titleBarStyle.height - titleBarStyle.marginBottom - titleBarStyle.marginTop - theme.toolbarHeight - toolbarStyle.marginTop - toolbarStyle.marginBottom;
		}
		...
		const toolbar = React.createElement(Toolbar, { style: toolbarStyle, items: toolbarItems });
		...
		const tagList = this.canDisplayTagBar() ? React.createElement(TagList, { style: tagStyle, items: this.state.noteTags }) : null;

		const titleBarMenuButton = React.createElement(IconButton, {
			style: {
				display: 'flex'
			},
		...
		const titleBarDate = React.createElement(
			'span',
			{ style: Object.assign({}, theme.textStyle, { color: theme.colorFaded }) },
			time.formatMsToLocal(note.user_updated_time)
		);

		const viewer = React.createElement(NoteTextViewer, { ref: this.webviewRef_, viewerStyle: viewerStyle, onDomReady: this.webview_domReady, onIpcMessage: this.webview_ipcMessage });
		...
		const noteSearchBarComp = !this.state.showLocalSearch ? null : React.createElement(NoteSearchBar, {
		...
		return React.createElement(
			'div',
			{ style: rootStyle, onDrop: this.onDrop_ },
			React.createElement(
				'div',
				{ style: titleBarStyle },
				titleEditor,
