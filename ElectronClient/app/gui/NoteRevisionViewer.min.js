...
class NoteRevisionViewerComponent extends React.PureComponent {
	...
	render() {
		...
		for (let i = 0; i < revs.length; i++) {
			...
			revisionListItems.push(React.createElement(
				'option',
				{ key: rev.id, value: rev.id },
		...
		const titleInput = React.createElement(
			'div',
			{ style: { display: 'flex', flexDirection: 'row', alignItems: 'center', marginBottom: 10, borderWidth: 1, borderBottomStyle: 'solid', borderColor: theme.dividerColor, paddingBottom: 10 } },
			React.createElement(
				'button',
				{ onClick: this.backButton_click, style: Object.assign({}, theme.buttonStyle, { marginRight: 10, height: theme.inputStyle.height }) },
				React.createElement('i', { style: theme.buttonIconStyle, className: 'fa fa-chevron-left' }),
				_('Back')
			),
			React.createElement('input', { readOnly: true, type: 'text', style: style.titleInput, value: this.state.note ? this.state.note.title : '' }),
			React.createElement(
				'select',
				{ disabled: !this.state.revisions.length, value: this.state.currentRevId, style: style.revisionList, onChange: this.revisionList_onChange },
				revisionListItems
			),
			React.createElement(
				'button',
				{ disabled: !this.state.revisions.length || this.state.restoring, onClick: this.importButton_onClick, style: Object.assign({}, theme.buttonStyle, { marginLeft: 10, height: theme.inputStyle.height }) },
				restoreButtonTitle
			),
			React.createElement(HelpButton, { tip: helpMessage, id: 'noteRevisionHelpButton', onClick: this.helpButton_onClick })
		);

		const viewer = React.createElement(NoteTextViewer, { viewerStyle: { display: 'flex', flex: 1, borderLeft: 'none' }, ref: this.viewerRef_, onDomReady: this.viewer_domReady, onIpcMessage: this.webview_ipcMessage });
		...
		return React.createElement(
			...
			titleInput,
			viewer,
			React.createElement(ReactTooltip, { place: 'bottom', delayShow: 300, className: 'help-tooltip' })
		);
	}
}
