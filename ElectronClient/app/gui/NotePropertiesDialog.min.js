...
class NotePropertiesDialog extends React.Component {
	...
	createNoteField(key, value) {
		...
		const labelComp = React.createElement(
			'label',
			{ style: Object.assign({}, theme.textStyle, { marginRight: '1em', width: '6em', display: 'inline-block', fontWeight: 'bold' }) },
		...
		if (this.state.editedKey === key) {
			if (key.indexOf('_time') >= 0) {
				controlComp = React.createElement(Datetime, {
					ref: 'editField',
					defaultValue: value,
				...
				controlComp = React.createElement('input', {
					defaultValue: value,
					type: 'text',
					ref: 'editField',
					onChange: event => {
			...
			if (['source_url', 'location'].indexOf(key) >= 0) {
				...
				controlComp = React.createElement(
					'a',
					{ href: '#', onClick: () => bridge().openExternal(url), style: theme.urlStyle },
					displayedValue
				);
			} else if (key === 'revisionsLink') {
				controlComp = React.createElement(
					'a',
					{ href: '#', onClick: this.revisionsLink_click, style: theme.urlStyle },
					_('Previous versions of this note')
				);
			} else {
				controlComp = React.createElement(
					'div',
					{ style: Object.assign({}, theme.textStyle, { display: 'inline-block' }) },
					displayedValue
				);
			}
		...
		if (editCompHandler) {
			editComp = React.createElement(
				'a',
				{ href: '#', onClick: editCompHandler, style: styles.editPropertyButton },
				React.createElement('i', { className: `fa ${editCompIcon}`, 'aria-hidden': 'true' })
			);
		}

		return React.createElement(
			'div',
			{ key: key, style: this.styles_.controlBox, className: 'note-property-box' },
			labelComp,
			controlComp,
			editComp
		);
	...
	render() {
		...
		return React.createElement(
			...
			React.createElement(
				'div',
				{ style: theme.dialogBox },
				React.createElement(
					'div',
					{ style: theme.dialogTitle },
					_('Note properties')
				),
				React.createElement(
					'div',
				...
				React.createElement(DialogButtonRow, { theme: this.props.theme, okButtonRef: this.okButton, onClick: this.buttonRow_click })
			)
		);
	}
}
