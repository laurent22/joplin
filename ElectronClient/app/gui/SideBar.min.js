...
class SideBarComponent extends React.Component {
	...
	folderItem(folder, selected, hasChildren, depth) {
		...
		const expandIcon = React.createElement('i', { style: expandIconStyle, className: `fa ${iconName}` });
		const expandLink = hasChildren ? React.createElement(
			'a',
			{ style: expandLinkStyle, href: '#', folderid: folder.id, onClick: this.onFolderToggleClick_ },
			expandIcon
		) : React.createElement(
			'span',
		...
		return React.createElement(
			'div',
			{ className: 'list-item-container', style: containerStyle, key: folder.id, onDragStart: this.onFolderDragStart_, onDragOver: this.onFolderDragOver_, onDrop: this.onFolderDrop_, draggable: true, folderid: folder.id },
			expandLink,
			React.createElement(
				'a',
				{
	...
	makeHeader(key, label, iconName, extraProps = {}) {
		...
		const icon = React.createElement('i', { style: { fontSize: style.fontSize, marginRight: 5 }, className: `fa ${iconName}` });
		...
		if (extraProps.toggleblock) {
			...
			toggleIcon = React.createElement('i', { className: `fa ${isExpanded ? 'fa-chevron-down' : 'fa-chevron-left'}`, style: { fontSize: style.fontSize * 0.75, marginRight: 12, marginLeft: 5, marginTop: style.fontSize * 0.125 } });
		}
	...
	synchronizeButton(type) {
		...
		const icon = React.createElement('i', { style: iconStyle, className: `fa ${iconName}` });
		return React.createElement(
			'a',
			{
	...
	render() {
		...
		if (this.props.folders.length) {
			...
			items.push(React.createElement(
				'div',
				{ className: 'folders', key: 'folder_items', style: { display: this.state.folderHeaderIsExpanded ? 'block' : 'none' } },
				folderItems
			));
		}
		...
		if (this.props.tags.length) {
			...
			items.push(React.createElement(
				'div',
				{ className: 'tags', key: 'tag_items', style: { display: this.state.tagHeaderIsExpanded ? 'block' : 'none' } },
				tagItems
			));
		}
		...
		for (let i = 0; i < lines.length; i++) {
			syncReportText.push(React.createElement(
				'div',
				{ key: i, style: { wordWrap: 'break-word', width: '100%' } },
				lines[i]
			));
		}
		...
		const syncReportComp = !syncReportText.length ? null : React.createElement(
			'div',
		...
		return React.createElement(
			'div',
			{ ref: this.rootRef, onKeyDown: this.onKeyDown, className: 'side-bar', style: style },
			React.createElement(
				'div',
