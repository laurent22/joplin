...
describe("Synchronizer", function() {
	...
	it(
		"should resolve folders conflicts",
		asyncTest(async () => {
			let folder1 = await Folder.save({ title: "folder1" });
			let note1 = await Note.save({ title: "un", parent_id: folder1.id });
			await synchronizer().start();
	...
	it(
		"should resolve conflict if remote folder has been deleted, but note has been added to folder locally",
		asyncTest(async () => {
			let folder1 = await Folder.save({ title: "folder1" });
			await synchronizer().start();
	...
	it(
		"should resolve conflict if note has been deleted remotely and locally",
		asyncTest(async () => {
	...
	it(
		"should handle conflict when remote note is deleted then local note is modified",
		asyncTest(async () => {
			let folder1 = await Folder.save({ title: "folder1" });
			let note1 = await Note.save({ title: "un", parent_id: folder1.id });
			await synchronizer().start();
	...
	it(
		"should handle conflict when remote folder is deleted then local folder is renamed",
		asyncTest(async () => {
			let folder1 = await Folder.save({ title: "folder1" });
			let folder2 = await Folder.save({ title: "folder2" });
			let note1 = await Note.save({ title: "un", parent_id: folder1.id });
	...
	it(
		"should not sync notes with conflicts",
		asyncTest(async () => {
			let f1 = await Folder.save({ title: "folder" });
	...
	it(
		"should not try to delete on remote conflicted notes that have been deleted",
		asyncTest(async () => {
			let f1 = await Folder.save({ title: "folder" });
	...
	it(
		"should not consider it is a conflict if neither the title nor body of the note have changed",
		asyncTest(async () => {
	...
	it(
		"should always handle conflict if local or remote are encrypted",
		asyncTest(async () => {
	...
	it(
		"items should be downloaded again when user cancels in the middle of delta operation",
		asyncTest(async () => {
			let folder1 = await Folder.save({ title: "folder1" });
			let note1 = await Note.save({ title: "un", is_todo: 1, parent_id: folder1.id });
			await synchronizer().start();
	...
	it(
		"should skip items that cannot be synced",
		asyncTest(async () => {
			let folder1 = await Folder.save({ title: "folder1" });
			let note1 = await Note.save({ title: "un", is_todo: 1, parent_id: folder1.id });
	...
	it(
		"notes and folders should get encrypted when encryption is enabled",
		asyncTest(async () => {
			Setting.setValue("encryption.enabled", true);
	...
	it(
		"should enable encryption automatically when downloading new master key (and none was previously available)",
		asyncTest(async () => {
	...
	it(
		"should upload decrypted items to sync target after encryption disabled",
		asyncTest(async () => {
			Setting.setValue("encryption.enabled", true);
			const masterKey = await loadEncryptionMasterKey();
	...
	it(
		"should not upload any item if encryption was enabled, and items have not been decrypted, and then encryption disabled",
		asyncTest(async () => {
	...
	it(
		"should encrypt remote resources after encryption has been enabled",
		asyncTest(async () => {
			while (insideBeforeEach) await time.msleep(100);
	...
	it(
		"should upload encrypted resource, but it should not mark the blob as encrypted locally",
		asyncTest(async () => {
			while (insideBeforeEach) await time.msleep(100);
	...
	it(
		"should create remote items with UTF-8 content",
		asyncTest(async () => {
