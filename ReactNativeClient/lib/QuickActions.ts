...
export default (dispatch: Function, folderId: string) => {
	// While `userInfo` is in theory supposed to be optional, if we don't include
	// both `userInfo` and `userInfo.url`, it crashes the app on Android (but not
	// on iPhone). Hopefully one day we can remove it. The bug is tracked here:
	// 	 https://github.com/jordanbyron/react-native-quick-actions/issues/90
	const userInfo = { url: '' };
