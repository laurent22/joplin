...
function updateOneItem(draft:Draft<State>, action:any, keyName:string = '') {
	...
	const newItems = (draft as any)[itemsKey].splice(0);
...
//const reducer = (state:State = defaultState, action:any) => {
	...
	try {
		...
		case 'NOTE_UPDATE_ONE':
			{
				...
				if (action.provisional) {
					draft.provisionalNoteIds.push(modNote.id);
				} else {
					const idx = draft.provisionalNoteIds.indexOf(modNote.id);
					if (idx >= 0) {
		...
		case 'TAG_DELETE':
			handleItemDelete(draft, action);
			draft.selectedNoteTags = removeItemFromArray(draft.selectedNoteTags.splice(0), 'id', action.id);
			break;
		...
		case 'NOTE_TAG_REMOVE':
			{
				...
				draft.selectedNoteTags = removeItemFromArray(draft.selectedNoteTags.splice(0), 'id', tagRemoved.id);
			}
			break;
		...
		case 'PLUGIN_DIALOG_SET':
			{
				...
				const newPlugins = Object.assign({}, draft.plugins);
				const newPlugin = draft.plugins[action.pluginName] ? Object.assign({}, draft.plugins[action.pluginName]) : {};
