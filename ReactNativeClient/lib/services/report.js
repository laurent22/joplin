...
class ReportService {
	...
	async basicItemList(option = null) {
		...
		for (let i = 0; i < itemTypes.length; i++) {
			...
			const items = await ItemClass.modelSelectAll(
				"SELECT items.id, items.updated_time, sync_items.sync_time FROM " + ItemClass.tableName() + " items JOIN sync_items ON sync_items.item_id = items.id"
			);
