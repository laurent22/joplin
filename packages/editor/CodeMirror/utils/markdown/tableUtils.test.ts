...
describe('tableUtils', () => {
	...
	describe('serializeTable', () => {
		...
		it('should preserve alignment markers', () => {
			const text = [
				'| Left | Center | Right |',
				'|:-----|:------:|------:|',
				'| a    | b      | c     |',
			...
			// Re-parse to verify alignment is preserved
			const reparsed = parseTable(result);
			expect(reparsed).not.toBeNull();
		...
		it('should pad columns for alignment', () => {
			...
			const result = serializeTable(table!);
			// The "A" column should not be padded to match "LongHeader"
			// but each column should be internally consistent
