...
3. There is two profiles now, one is `full` which runs Joplin Server and the Transcribe server, and other is `server` which is only Joplin Server
4. To choose the profile that you want pass the `--profile server` option when using docker compose
