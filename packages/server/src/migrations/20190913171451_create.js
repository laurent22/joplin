...
const up = (db) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    yield db.schema.createTable('sessions', (table) => {
        table.string('id', 32).unique().primary().notNullable();
    ...
    yield db.schema.createTable('permissions', (table) => {
        table.string('id', 32).unique().primary().notNullable();
        table.string('user_id', 32).notNullable();
    ...
    yield db.schema.alterTable('permissions', (table) => {
        table.unique(['user_id', 'item_type', 'item_id']);
        table.index(['item_id']);
        table.index(['item_type', 'item_id']);
    });
    yield db.schema.createTable('files', (table) => {
        table.string('id', 32).unique().primary().notNullable();
    ...
    yield db.schema.alterTable('files', (table) => {
        table.unique(['parent_id', 'name']);
        table.index(['parent_id']);
    });
    yield db.schema.createTable('changes', (table) => {
    ...
    yield db.schema.createTable('api_clients', (table) => {
        table.string('id', 32).unique().primary().notNullable();
