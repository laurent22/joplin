...
const up = (db) => __awaiter(void 0, void 0, void 0, function* () {
    yield db.schema.createTable('items', (table) => {
    ...
    yield db.schema.createTable('user_items', (table) => {
    ...
    yield db.schema.alterTable('user_items', (table) => {
        table.unique(['user_id', 'item_id']);
        table.index('user_id');
        table.index('item_id');
    });
    yield db.schema.createTable('item_resources', (table) => {
        table.increments('id').unique().primary().notNullable();
        table.string('item_id', 32).notNullable();
        table.string('resource_id', 32).notNullable();
    });
    yield db.schema.alterTable('item_resources', (table) => {
        table.unique(['item_id', 'resource_id']);
        table.index(['item_id', 'resource_id']);
    });
    yield db.schema.createTable('key_values', (table) => {
        table.increments('id').unique().primary().notNullable();
        table.text('key').notNullable();
        table.integer('type').notNullable();
        table.text('value').notNullable();
    });
    yield db.schema.alterTable('key_values', (table) => {
        table.index(['key']);
    });
    ...
    yield db.schema.alterTable('changes', (table) => {
