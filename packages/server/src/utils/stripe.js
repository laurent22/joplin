...
function priceIdToAccountType(priceId) {
    const price = (0, joplinCloud_1.findPrice)(stripeConfig(), { priceId });
    return price.accountType;
}
exports.priceIdToAccountType = priceIdToAccountType;
function accountTypeToPriceId(accountType) {
    const price = (0, joplinCloud_1.findPrice)(stripeConfig(), { accountType, period: joplinCloud_1.PricePeriod.Monthly });
    return price.id;
}
...
function subscriptionInfoByUserId(models, userId) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!sub)
            throw new errors_1.ErrorWithCode('Could not retrieve subscription info', errors_1.ErrorCode.NoSub);
        const stripe = initStripe();
        const stripeSub = yield stripe.subscriptions.retrieve(sub.stripe_subscription_id);
        if (!stripeSub)
            throw new errors_1.ErrorWithCode('Could not retrieve Stripe subscription', errors_1.ErrorCode.NoStripeSub);
...
function updateSubscriptionType(models, userId, newAccountType) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const currentPrice = (0, joplinCloud_1.findPrice)(stripeConfig(), { priceId: stripePriceIdByStripeSub(stripeSub) });
        const upgradePrice = (0, joplinCloud_1.findPrice)(stripeConfig(), { accountType: newAccountType, period: currentPrice.period });
        const items = [];
