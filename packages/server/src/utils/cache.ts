...
class Cache {
	...
	private async setAny(key: string, o: unknown): Promise<void> {
		this.cache[key] = {
	...
	public async setObject(key: string, object: unknown): Promise<void> {
		if (!object) return;
		return this.setAny(key, object);
	}

	private async getAny(key: string): Promise<unknown> {
