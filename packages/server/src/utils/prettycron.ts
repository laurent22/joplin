...
(function() {
	...
	const numberList = function(numbers: number[]) {
		if (numbers.length < 2) {
	...
	const stepSize = function(numbers: number[]) {
	...
	const isEveryOther = function(stepsize: number, numbers: number[]) {
		return numbers.length === 30 && stepsize === 2;
	};
	const isTwicePerHour = function(stepsize: number, numbers: number[]) {
		return numbers.length === 2 && stepsize === 30;
	};
	const isOnTheHour = function(numbers: number[]) {
		return numbers.length === 1 && numbers[0] === 0;
	};
	const isStepValue = function(stepsize: number, numbers: number[]) {
	...
	const getMinutesTextParts = function(numbers: number[]) {
	...
	// listing all the values unless they represent a frequency divisible by 60:
	// /2, /3, /4, /5, /6, /10, /12, /15, /20 and /30
	const getSecondsTextParts = function(numbers: number[]) {
		const stepsize = stepSize(numbers);
		if (!numbers) {
	...
	const numberToDateName = function(value: number | string, type: 'dow' | 'mon') {
		if (type === 'dow') {
			return dayjs().day(Number(value) - 1).format('ddd');
		} else if (type === 'mon') {
			return dayjs().month(Number(value) - 1).format('MMM');
		}
	};
	...
	const dateList = function(numbers: number[], type: 'dow' | 'mon') {
		if (numbers.length < 2) {
	...
	type LaterSchedule = Record<string, number[]>;

	const removeFromSchedule = function(schedule: LaterSchedule, member: string, length: number) {
	...
	const scheduleToSentence = function(schedule: LaterSchedule, useSeconds: boolean) {
		let textParts = [];
	...
	const toString = function(cronspec: string, sixth: boolean) {
	...
	const getNextDate = function(cronspec: string, sixth: boolean) {
	...
	const getNext = function(cronspec: string, sixth: boolean) {
	...
	// (This is just a wrapper for later.js and moment.js)
	const getNextDates = function(cronspec: string, numDates: number, sixth: boolean) {
		const schedule = later.parse.cron(cronspec, sixth);
	...
	const global_obj = (typeof exports !== 'undefined' && exports !== null) ? exports : (window as Window & { prettyCron?: Record<string, unknown> }).prettyCron = {};
