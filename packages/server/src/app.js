...
const env_2 = require("@joplin/utils/env");
const storageConnectionCheck_1 = require("./utils/storageConnectionCheck");
const locale_1 = require("@joplin/lib/locale");
const initLib_1 = require("@joplin/lib/initLib");
...
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            if (envFilePath)
                envFromFile = (0, env_2.parseEnvFile)(envFilePath);
        }
        catch (error) {
            error.message = `Could not parse env file at ${envFilePath}: ${error.message}`;
            throw error;
        }
        const fullEnv = Object.assign(Object.assign({}, envFromFile), process.env);
        if (!defaultEnvVariables[env])
            throw new Error(`Invalid env: ${env}`);
        const envVariables = (0, env_1.parseEnv)(fullEnv, defaultEnvVariables[env]);
        const app = new Koa();
        ...
        if (selectedCommand) {
            ...
            appLogger().info(`Starting server ${(0, config_1.fullVersionString)((0, config_1.default)())} (${env}) on port ${(0, config_1.default)().port} and PID ${process.pid}...`);
            if ((0, config_1.default)().maxTimeDrift) {
                appLogger().info(`Checking for time drift using NTP server: ${(0, config_1.default)().NTP_SERVER}`);
                const timeDrift = yield (0, ntp_1.getDeviceTimeDrift)((0, config_1.default)().NTP_SERVER);
            ...
            if ((0, config_1.default)().database.autoMigration) {
                ...
                if (!(0, config_1.default)().DB_ALLOW_INCOMPLETE_MIGRATIONS && (yield (0, db_1.needsMigration)(connectionCheck.connection))) {
                    const list = yield (0, db_1.migrateList)(connectionCheck.connection, true);
            ...
            yield (0, startServices_1.default)((0, config_1.default)(), ctx.joplinBase.services);
