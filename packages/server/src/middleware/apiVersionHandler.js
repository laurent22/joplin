...
function default_1(ctx, next) {
    return __awaiter(this, void 0, void 0, function* () {
        if (!(0, requestUtils_1.isApiRequest)(ctx))
            return next();
        const joplinServerVersion = (0, config_1.default)().joplinServerVersion;
        const minVersion = ctx.headers['x-api-min-version'];
        // For now we don't require this header to be set to keep compatibility with
        // older clients.
        if (!minVersion)
            return next();
        const diff = (0, compare_versions_1.compareVersions)(joplinServerVersion, minVersion);
        ...
        if (diff < 0) {
            throw new errors_1.ErrorPreconditionFailed(`Joplin Server v${minVersion} is required but v${joplinServerVersion} is installed. Please upgrade Joplin Server.`);
        }
        return next();
    });
}
