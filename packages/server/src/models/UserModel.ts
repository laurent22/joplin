...
export default class UserModel extends BaseModel<User> {
	...
	public async handleOversizeAccounts() {
		...
		await this.withTransaction(async () => {
			for (const user of users) {
				...
				if (user.total_item_size > maxTotalItemSize * alertLimitMax) {
					...
					// Users who are over the limit can no longer upload data,
					// however we don't disable the account so that they can fix
					// it by deleting notes and resources.
					await this.save({
						id: user.id,
						can_upload: 0,
					});
