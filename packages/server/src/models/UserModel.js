...
class UserModel extends BaseModel_1.default {
    ...
    login(email, password) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!user)
                return null;
            if (!(0, auth_1.checkPassword)(password, user.password))
                return null;
            return user;
        });
    }
    fromApiInput(object) {
        ...
        if ('can_receive_folder' in object)
            user.can_receive_folder = object.can_receive_folder;
    ...
    save(object, options = {}) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (user.password) {
                if ((0, auth_1.isHashedPassword)(user.password)) {
                    throw new errors_1.ErrorBadRequest(`Unable to save user because password already seems to be hashed. User id: ${user.id}`);
                }
                if (!options.skipValidation)
                    this.validatePassword(user.password);
                user.password = (0, auth_1.hashPassword)(user.password);
            }
