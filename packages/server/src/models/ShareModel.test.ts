import { createUserAndSession, beforeAllDb, afterAllTests, beforeEachDb, models, checkThrowAsync, createItem, deleteItem, updateItem, createItemTree, expectNotThrow, createNote } from '../utils/testing/testUtils';
...
describe('ShareModel', () => {
	...
	test('benchmark updateSharedItems3', async () => {
		const { session: session1, user: _user1 } = await createUserAndSession(1);
		const { session: session2, user: user2 } = await createUserAndSession(2);
		...
		const moveToShare = async (item: Item, share: Share) => {
			return await updateItemShareId(session1, item.id, share.id);
		};
		...
		const iterateChangeData = async function*() {
			...
			{ // Scenario 3
				...
				for (let i = 0; i < 100; i++) {
					const s = await createShare();
					items.push(await createNote(s));
				}

				const s2 = await createShare();
				for (const item of items) {
					await moveToShare(item, s2);
				}
