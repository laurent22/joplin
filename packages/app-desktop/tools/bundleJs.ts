		...
		plugins: [
			{
				...
				setup: build => {
					const externalRegex = /^(.*\.node|sqlite3|@journeyapps\/sqlcipher(?:\/.*)?|node-fetch|electron|@electron\/remote\/.*|electron\/.*|@mapbox\/node-pre-gyp|jsdom|onnxruntime-node|@xenova\/transformers)$/;
