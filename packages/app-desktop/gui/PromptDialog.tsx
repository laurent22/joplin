...
export default class PromptDialog extends React.Component<Props, any> {
	...
	render() {
		...
		const handleHtmlParsedLabel = (label:string) => {
			return (
				<div>
		...
		if (this.props.inputType === 'datetime') {
			...
			inputComp = <Select className="item-selector" styles={styles.select} theme={styles.selectTheme} ref={this.answerInput_} components={makeAnimated()} value={this.props.answer} defaultValue={this.props.defaultValue} isClearable={false} options={this.props.autocomplete} formatOptionLabel={handleHtmlParsedLabel} onChange={onSelectChange} onKeyDown={(event: any) => onKeyDown(event)} />;
		} else {
