		...
		const loadContent = async () => {
			...
			if (differentNoteId || differentContent || !resourcesEqual) {
				...
				if (!hasOneNoteInkContentStyle && new DOMParser().parseFromString(result.html, 'text/html').querySelector('.ink-text, .ink-space, .container-outline')) {
					...
					styleElement.textContent = '.ink-text, .ink-space { display: inline-block; position: relative; vertical-align: bottom; } .container-outline:has(.ink-text, .ink-space) { font-family: Calibri, sans-serif; font-size: 6pt; font-weight: normal; }';
