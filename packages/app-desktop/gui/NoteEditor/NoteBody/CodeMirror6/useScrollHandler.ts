...
const useScrollHandler = (
	editorRef: RefObject<CodeMirrorControl>, webviewRef: any, onScroll: OnScrollCallback,
) => {
	...
	const editor_resize = useCallback((cm: CodeMirrorControl) => {
		if (isCodeMirrorReady(cm)) {
	...
	const editor_update = useCallback((cm: CodeMirrorControl, edited: boolean) => {
		if (isCodeMirrorReady(cm)) {
			if (edited) return;
...
const translateLE_ = (codeMirror: CodeMirrorControl, percent: number, l2e: boolean) => {
...
const translateScrollPercentL2E = (cm: CodeMirrorControl, lPercent: number) => {
	return translateLE_(cm, lPercent, true);
};

const translateScrollPercentE2L = (cm: CodeMirrorControl, ePercent: number) => {
	return translateLE_(cm, ePercent, false);
};

function isCodeMirrorReady(cm: CodeMirrorControl) {
	const info = cm?.getScrollInfo();
