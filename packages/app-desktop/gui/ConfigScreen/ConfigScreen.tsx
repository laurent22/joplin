...
class ConfigScreenComponent extends React.Component<any, any> {
	...
	private onSearchChange = (event: { value: string }) => {
	...
	public sectionToComponent(
		key: string,
		section: { name: string; metadatas: { key: string; advanced?: boolean }[] },
		...
		if (section.name === 'sync') {
			const syncTargetMd = SyncTargetRegistry.idToMetadata(
				settings['sync.target'] as number,
			);
        ...
        settings['sync.target'] === this.props.settings['sync.target'];
			if (
				matchesSavedTarget &&
        shouldShowMissingPasswordWarning(settings['sync.target'] as number, settings)
			) {
				settingComps.push(
