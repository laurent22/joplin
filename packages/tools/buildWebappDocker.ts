...
function parseArgv(): Argv {
	return require('yargs')
		.scriptName('yarn buildWebappDocker')
		...
		.option('t', {
			alias: 'tagName',
			describe: 'Base image tag. Usually should be in format `webapp-v1.2.3` or `webapp-v1.2.3-beta`. The latest `webapp-v*` git tag will be used by default.',
			type: 'string',
		})
		.option('l', {
			alias: 'addLatestTag',
...
async function main() {
	...
	const tagName = argv.tagName || `webapp-${await execCommand('git describe --tags --match v*', { showStdout: false })}`;
	...
	cliArgs.push('-f Dockerfile.webapp');
	cliArgs.push('.');
