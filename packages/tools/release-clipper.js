...
async function compareDir(dir1, dir2) {
	...
	const files1 = glob.sync(`${dir1}/**/*`, globOptions).filter(filterFiles).map(f => f.slice(dir1.length + 1));
	const files2 = glob.sync(`${dir2}/**/*`, globOptions).filter(filterFiles).map(f => f.slice(dir2.length + 1));
