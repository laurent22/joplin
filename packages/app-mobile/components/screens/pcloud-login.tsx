...
class PCloudLoginScreenComponent extends BaseScreenComponent<Props, State> {
	...
	public render() {
		...
		return (
			<View style={this.styles().screen}>
				<ScreenHeader title={_('Login with pCloud')} />

				<ScrollView style={this.styles().container}>
					<Text style={this.styles().stepText}>{_('To allow Joplin to synchronise with pCloud, please follow the steps below:')}</Text>
					...
					<Text style={this.styles().stepText}>{_('Step 2: Enter the code provided by pCloud:')}</Text>
...
const PCloudLoginScreen = connect((state: AppState) => {
	return {
		themeId: state.settings.theme,
	};
})(PCloudLoginScreenComponent);

export default PCloudLoginScreen;
