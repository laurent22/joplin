...
describe('JobProcessor', () => {
	...
	skipByDefault('should execute work on job in the queue', async () => {
		jest.useRealTimers();
		const tw = new JobProcessor(queue, new HtrCli({ htrCliImagesFolder: join(process.cwd(), 'images'), htrCliDockerImage: 'joplin/htr-cli:latest' }), new FileStorage(), 1000);
		await tw.init();
	...
	skipByDefault('should remove file sent to queue if job fails too many times', async () => {
		...
		const tw = new JobProcessor(queue, new HtrCli({ htrCliImagesFolder: join(process.cwd(), 'images'), htrCliDockerImage: 'joplin/htr-cli:latest' }), fileStorage, 1000);
		await tw.init();
