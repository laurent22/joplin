	...
	it('should make tool calls compatible with the Anthropic API', () => {
		const { turnMessages } = _internal.convertMessages([
			{ role: ChatRole.User, content: 'Do something' },
			{
				...
				content: 'okay',
				toolCalls: [
					{ toolName: 'replaceRange', callId: 'call-1', arguments: { text: 'test', anchor: 'original' }, parseError: undefined },
					{ toolName: 'replaceRange', callId: 'call-2', arguments: { text: 'test-2', anchor: 'other' }, parseError: undefined },
				],
			},
			{
				role: ChatRole.Tool,
