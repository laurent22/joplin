...
const checkRandomBytes = async () => {
	const length = 100;
	const randomBytes = await shim.randomBytes(length);
	const errorPrefix = `shim.randomBytes(${length}) test failed`;

	if (randomBytes.length !== length) {
		throw new Error(`${errorPrefix}: unexpected length ${randomBytes.length}`);
	}
	...
	for (const b of randomBytes) {
		...
		if (!Number.isInteger(b)) {
			throw new Error(`${errorPrefix}: Generated byte is not an integer: ${b}`);
		}
		if (b < 0 || b > 255) {
			throw new Error(`${errorPrefix}: Byte out of range: ${b}`);
		}
	}
