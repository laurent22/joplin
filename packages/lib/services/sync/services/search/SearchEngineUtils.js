...
class SearchEngineUtils {
    static async notesForQuery(query, applyUserSettings, options = null, searchEngine = null) {
        options = Object.assign({ appendWildCards: false }, options);
        if (!searchEngine) {
        ...
        const previewOptions = Object.assign({ order: [], fields: fields, conditions: [`id IN (${Note_1.default.escapeIdsForSql(noteIds)})`] }, options);
        const notes = await Note_1.default.previews(null, previewOptions);
