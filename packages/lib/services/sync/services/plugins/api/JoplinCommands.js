 ...
 *
 */
class JoplinCommands {
    constructor(plugin_) {
        this.plugin_ = plugin_;
    }
    /**
    ...
    async execute(commandName, ...args) {
        return CommandService_1.default.instance().execute(commandName, ...args);
    }
    /**
    ...
    async register(command) {
        ...
        if ('iconName' in command)
            declaration.iconName = command.iconName;
        const runtime = {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Old code before rule was applied
        ...
        if ('enabledCondition' in command)
            runtime.enabledCondition = command.enabledCondition;
        CommandService_1.default.instance().registerDeclaration(declaration);
        CommandService_1.default.instance().registerRuntime(declaration.name, runtime);
        this.plugin_.addOnUnloadListener(() => {
            CommandService_1.default.instance().unregisterRuntime(declaration.name);
            CommandService_1.default.instance().unregisterDeclaration(declaration.name);
        });
    }
}
exports.default = JoplinCommands;
