...
exports.enexXmlToMd = enexXmlToMd;
exports.processMdArrayNewLines = processMdArrayNewLines;
...
(function (ListTag) {
    ListTag["Ul"] = "ul";
    ListTag["Ol"] = "ol";
    ListTag["CheckboxList"] = "checkboxList";
    ListTag["TaskList"] = "taskList";
})(ListTag || (ListTag = {}));
...
function simplifyString(s) {
    ...
    while (output.length && isWhiteSpace(output[0]))
        output = output.substr(1);
    while (output.length && isWhiteSpace(output[output.length - 1]))
...
const addResourceTag = (lines, src, mime, options) => {
    const alt = options.alt ? tagAttributeToMdText(options.alt) : '';
    if (resourceUtils.isImageMimeType(mime)) {
        if (!!options.width || !!options.height) {
            const attrs = { src };
            if (options.width)
                attrs.width = options.width.toString();
            if (options.height)
                attrs.height = options.height.toString();
            if (alt)
                attrs.alt = alt;
            const attrsHtml = [];
            for (const [key, value] of Object.entries(attrs)) {
                attrsHtml.push(`${key}="${(0, html_1.htmlentities)(value)}"`);
            }
            ...
            lines.push('![');
            lines.push(alt);
            lines.push(`](${markdownUtils_1.default.escapeLinkUrl(src)})`);
        }
    }
    else {
        lines.push('[');
...
const altFromResource = (resource) => {
    let alt = '';
    if (!alt)
        alt = resource.title;
    if (!alt)
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any -- Old code before rule was applied
function cssValue(context, style, propName) {
    if (!style)
        return null;
    ...
    try {
        ...
        for (const propName of propNames) {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Old code before rule was applied
            const prop = o.stylesheet.rules[0].declarations.find((d) => d.property.toLowerCase() === propName);
            if (prop && prop.value)
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any -- Old code before rule was applied
function isCodeBlock(context, nodeName, attributes) {
    if (nodeName === 'code')
        return true;
    if (attributes && attributes.style) {
        ...
        if (enCodeBlock && enCodeBlock.toLowerCase() === 'true')
            return true;
    }
    return false;
}
// eslint-disable-next-line @typescript-eslint/no-explicit-any -- Old code before rule was applied
function isHighlight(context, _nodeName, attributes) {
    if (attributes && attributes.style) {
        ...
        // "false".
        if (enHighlight && enHighlight.toLowerCase() !== 'false')
            return true;
    }
    return false;
}
// eslint-disable-next-line @typescript-eslint/no-explicit-any -- Old code before rule was applied
function enexXmlToMdArray(stream, resources, tasks) {
    ...
    return new Promise((resolve) => {
        ...
        saxStream.on('text', (text) => {
            if (['table', 'tr', 'tbody'].indexOf(section.type) >= 0)
                return;
        ...
        saxStream.on('opentag', function (node) {
            ...
            if (currentList) {
                if (!currentList.startedText && isBlockTag(n))
                    return;
            ...
            if (n === 'en-note') {
                ...
                const tag = isCheckboxList ? ListTag.CheckboxList : n;
            ...
            if (n === 'en-note') {
                ...
                if (!state.inCode.length) {
                    ...
                    if (codeLines.length > 1) {
                        ...
                        section.lines.push(`\`${markdownUtils_1.default.escapeInlineCode(codeLines.join(''))}\``);
                    }
                    if (section && section.parent)
                        section = section.parent;
                }
                ...
                if (section.lines.length < 1)
                    throw new Error('Invalid anchor tag closing'); // Sanity check, but normally not possible
...
async function enexXmlToMd(xmlString, resources, tasks) {
