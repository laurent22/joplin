...
const addMigrationFile_1 = require("./services/database/addMigrationFile");
const sqlStringToLines_1 = require("./services/database/sqlStringToLines");
const migrations_1 = require("./services/database/migrations");
...
`;
class JoplinDatabase extends database_1.default {
    ...
    tableFieldNames(tableName) {
        if (this.tableFieldNames_[tableName])
            return this.tableFieldNames_[tableName].slice();
    ...
    tableFields(tableName, options = null) {
        if (options === null)
            options = {};
        if (!this.tableFields_)
            throw new Error('Fields have not been loaded yet');
        if (!this.tableFields_[tableName])
    ...
    createDefaultRow(tableName) {
        ...
        for (let i = 0; i < fields.length; i++) {
            const f = fields[i];
            row[f.name] = database_1.default.formatValue(f.type, f.default);
        }
        return row;
    }
    ...
    async refreshTableFields(newVersion) {
        ...
        for (let i = 0; i < tableRows.length; i++) {
            ...
            if (tableName === 'sqlite_sequence')
                continue;
            if (tableName.indexOf('notes_fts') === 0)
                continue;
            if (tableName.indexOf('items_fts') === 0)
                continue;
            if (tableName === 'notes_spellfix')
                continue;
            if (tableName === 'search_aux')
                continue;
            const pragmas = await this.selectAll(`PRAGMA table_info("${tableName}")`);
            for (let i = 0; i < pragmas.length; i++) {
                ...
                const q = database_1.default.insertQuery('table_fields', {
                    table_name: tableName,
                    field_name: item.name,
                    field_type: database_1.default.enumId('fieldType', item.type),
    ...
    async upgradeDatabase(fromVersion) {
        ...
        for (let i = 0; i < migrations_1.default.length; i++)
            existingDatabaseVersions.push(existingDatabaseVersions[existingDatabaseVersions.length - 1] + 1);
        ...
        if (currentVersionIndex < 0) {
            throw new Error('Unknown profile version. Most likely this is an old version of Joplin, while the profile was created by a newer version. Please upgrade Joplin at https://joplinapp.org and try again.\n'
                + `Joplin version: ${shim_1.default.appVersion()}\n`
        ...
        while (currentVersionIndex < existingDatabaseVersions.length - 1) {
            ...
            let queries = [];
            if (targetVersion === 1) {
                queries = this.wrapQueries((0, sqlStringToLines_1.default)(structureSql));
            }
            if (targetVersion === 2) {
                const newTableSql = `
					CREATE TABLE deleted_items (
                ...
                queries.push({ sql: (0, sqlStringToLines_1.default)(newTableSql)[0] });
            ...
            if (targetVersion === 9) {
				...
				`;
                queries.push((0, sqlStringToLines_1.default)(newTableSql)[0]);
                const tableNames = ['notes', 'folders', 'tags', 'note_tags', 'resources'];
                for (let i = 0; i < tableNames.length; i++) {
                    const n = tableNames[i];
            ...
            const upgradeVersion10 = () => {
				...
				`;
                queries.push((0, sqlStringToLines_1.default)(itemChangesTable)[0]);
                ...
                queries.push((0, sqlStringToLines_1.default)(noteResourcesTable)[0]);
            ...
            if (targetVersion === 13) {
                ...
                queries.push({ sql: 'UPDATE resources SET fetch_status = ?', params: [Resource_1.default.FETCH_STATUS_DONE] });
            }
            if (targetVersion === 14) {
				...
				`;
                queries.push((0, sqlStringToLines_1.default)(resourceLocalStates)[0]);
            ...
            if (targetVersion === 18) {
				...
				`;
                queries.push((0, sqlStringToLines_1.default)(notesNormalized)[0]);
            ...
            if (targetVersion === 20) {
                ...
                queries.push((0, addMigrationFile_1.default)(20));
            }
            if (targetVersion === 21) {
            ...
            if (targetVersion === 27) {
                queries.push((0, addMigrationFile_1.default)(27));
            }
            if (targetVersion === 28) {
            ...
            if (targetVersion === 30) {
                ...
                queries = queries.concat(this.alterColumnQueries('notes', {
                    id: 'TEXT PRIMARY KEY',
                    parent_id: 'TEXT NOT NULL DEFAULT ""',
                    title: 'TEXT NOT NULL DEFAULT ""',
                    body: 'TEXT NOT NULL DEFAULT ""',
            ...
            if (targetVersion === 33) {
                ...
                queries.push((0, sqlStringToLines_1.default)(newVirtualTableSql)[0]);
                queries.push(`
					CREATE TRIGGER notes_fts_before_update BEFORE UPDATE ON notes_normalized BEGIN
						DELETE FROM notes_fts WHERE docid=old.rowid;
					END;`);
                ...
                queries.push(`
					CREATE TRIGGER notes_after_insert AFTER INSERT ON notes_normalized BEGIN
						INSERT INTO notes_fts(docid, ${tableFields}) SELECT rowid, ${tableFields} FROM notes_normalized WHERE new.rowid = notes_normalized.rowid;
					END;`);
                queries.push((0, addMigrationFile_1.default)(33));
            }
            if (targetVersion === 34) {
            ...
            if (targetVersion === 35) {
                ...
                queries.push((0, addMigrationFile_1.default)(35));
            }
            if (targetVersion === 36) {
            ...
            if (targetVersion > 41) {
                const migration = migrations_1.default[targetVersion - 42];
                if (!migration)
                    throw new Error(`No such migration: ${targetVersion}`);
    ...
    async initialize() {
        ...
        if (newVersion !== tableFieldsVersion)
            await this.refreshTableFields(newVersion);
        this.tableFields_ = {};
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            this.tableFields_[row.table_name].push({
                name: row.field_name,
                type: row.field_type,
                default: database_1.default.formatValue(row.field_type, row.field_default),
            });
        }
    }
}
JoplinDatabase.TYPE_INT = 1;
JoplinDatabase.TYPE_TEXT = 2;
JoplinDatabase.TYPE_NUMERIC = 3;
exports.default = JoplinDatabase;
