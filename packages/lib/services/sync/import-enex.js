...
const markdown_1 = require("@joplin/utils/markdown");
...
let fs_ = null;
const fs = () => {
    fs_ !== null && fs_ !== void 0 ? fs_ : (fs_ = shim_1.default.requireDynamic('fs-extra'));
    return fs_;
};
...
async function processNoteResource(resource) {
    const handleNoDataResource = async (resource, setId) => {
        if (setId)
            resource.id = md5(Date.now() + Math.random());
        resource.size = 0;
        resource.dataFilePath = `${Setting_1.default.value('tempDir')}/${resource.id}.empty`;
        await shim_1.default.fsDriver().writeFile(resource.dataFilePath, '');
    };
    if (!resource.hasData) {
        ...
        const stats = await shim_1.default.fsDriver().stat(resource.dataFilePath);
        ...
        if (!resource.id) {
            ...
            resource.id = await shim_1.default.fsDriver().md5File(resource.dataFilePath);
        }
...
async function saveNoteResources(note) {
    ...
    for (let i = 0; i < note.resources.length; i++) {
        ...
        toSave.file_extension = resource.filename ? (0, path_utils_1.safeFileExtension)((0, path_utils_1.fileExtension)(resource.filename)) : '';
        ...
        toSave.filename = toSave.filename ? (0, path_utils_1.friendlySafeFilename)(toSave.filename, 255, true) : '';
        ...
        // same resource.
        const existingResource = await Resource_1.default.load(toSave.id);
        if (existingResource)
            continue;
        await shim_1.default.fsDriver().move(resource.dataFilePath, Resource_1.default.fullPath(toSave));
        await Resource_1.default.save(toSave, { isNew: true });
        resourcesCreated++;
    }
...
async function saveNoteTags(note) {
    ...
    for (let i = 0; i < note.tags.length; i++) {
        ...
        let tag = await Tag_1.default.loadByTitle(tagTitle);
        if (!tag)
            tag = await Tag_1.default.save({ title: tagTitle });
        await Tag_1.default.addNote(tag.id, note.id);
        notesTagged++;
    }
    return notesTagged;
}
async function saveNoteToStorage(note) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Old code before rule was applied
    note = Note_1.default.filter(note);
    const result = {
        resourcesCreated: 0,
        notesTagged: 0,
    };
    ...
    await Note_1.default.save(note, {
        isNew: true,
        autoTimestamp: false,
    });
    return result;
}
const preProcessFile = async (filePath) => {
...
const isEvernoteUrl = (url) => {
    return url.toLowerCase().startsWith('evernote://');
};
const restoreNoteLinks = async (notes, noteTitlesToIds, importOptions) => {
    // --------------------------------------------------------
    ...
    for (const note of notes) {
        const links = importOptions.outputFormat === 'html' ?
            (0, html_1.extractUrls)(note.body) :
            (0, markdown_1.extractUrls)(note.body);
        let noteChanged = false;
        for (const link of links) {
            if (!isEvernoteUrl(link.url))
                continue;
...
const parseNotes = async (parentFolderId, filePath, importOptions = null) => {
    ...
    return new Promise((resolve, reject) => {
            ...
            while (notes.length) {
                ...
                try {
                    ...
                    const body = importOptions.outputFormat === 'html' ?
                        ...
                        await (0, import_enex_md_gen_1.enexXmlToMd)(note.bodyXml, note.resources, note.tasks);
                    delete note.bodyXml;
                    // --------------------------------------------------------
                    // Finish setting up the note
                    // --------------------------------------------------------
                    note.id = uuid_1.default.create();
                    note.markup_language = importOptions.outputFormat === 'html' ?
                        renderer_1.MarkupToHtml.MARKUP_LANGUAGE_HTML :
                        renderer_1.MarkupToHtml.MARKUP_LANGUAGE_MARKDOWN;
                    ...
                    if (!noteTitlesToIds[note.title])
                        noteTitlesToIds[note.title] = [];
            ...
            if (noteAttributes) {
                ...
                if (n === 'data') {
                    ...
                    if (!noteResource.dataFilePath) {
                        noteResource.dataFilePath = `${Setting_1.default.value('tempDir')}/${md5(Date.now() + Math.random())}.base64`;
                    }
        ...
        saxStream.on('opentag', handleSaxStreamEvent((node) => {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Old code before rule was applied
        saxStream.on('cdata', handleSaxStreamEvent((data) => {
            const n = currentNodeName();
        ...
        saxStream.on('closetag', handleSaxStreamEvent(function (n) {
            ...
            if (n === 'note') {
                ...
                if (noteResourceAttributes['source-url'])
                    noteResource.sourceUrl = noteResourceAttributes['source-url'];
        ...
        saxStream.on('end', handleSaxStreamEvent(() => {
            ...
            const iid = shim_1.default.setInterval(() => {
                ...
                void processNotes().then(allDone => {
                    if (allDone) {
                        shim_1.default.clearTimeout(iid);
                        if (needToDeleteFileToProcess)
                            void shim_1.default.fsDriver().remove(fileToProcess);
                        resolve({
                            savedNotes,
                            noteTitlesToIds,
                        });
...
async function importEnex(parentFolderId, filePath, importOptions = null) {
    if (!importOptions)
        importOptions = {};
    if (!('onProgress' in importOptions))
        importOptions.onProgress = function () { };
    if (!('onError' in importOptions))
        importOptions.onError = function () { };
    if (!('batchSize' in importOptions))
        importOptions.batchSize = 10;
