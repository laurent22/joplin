...
const joplinCloudUtils_1 = require("./services/joplinCloudUtils");
...
class JoplinServerApi {
    ...
    baseUrl() {
        return (0, path_utils_1.rtrimSlashes)(this.options_.baseUrl());
    }
    ...
    async getClientInfo() {
        const { platform, type } = await (0, joplinCloudUtils_1.getApplicationInformation)();
        const clientInfo = {
            platform,
            type,
    ...
    async session() {
        ...
        try {
            this.session_ = await this.exec_('POST', 'api/sessions', null, Object.assign({ email: this.options_.username(), password: this.options_.password() }, clientInfo));
            return this.session_;
        }
        catch (error) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Old code before rule was applied
    async exec_(method, path = '', query = null, body = null, headers = null, options = null) {
        if (headers === null)
            headers = {};
        if (options === null)
            options = {};
        ...
        try {
            ...
            if (options.source === 'file' && (method === 'POST' || method === 'PUT')) {
                if (fetchOptions.path) {
                    const fileStat = await shim_1.default.fsDriver().stat(fetchOptions.path);
                    if (fileStat)
                        fetchOptions.headers['Content-Length'] = `${fileStat.size}`;
                }
                response = await shim_1.default.uploadBlob(url, fetchOptions);
            }
            else if (options.target === 'string') {
                if (typeof body === 'string')
                    fetchOptions.headers['Content-Length'] = `${shim_1.default.stringByteLength(body)}`;
                response = await shim_1.default.fetch(url, fetchOptions);
            }
            else {
                // file
                response = await shim_1.default.fetchBlob(url, fetchOptions);
            }
            ...
            if (!response.ok) {
                if (options.target === 'file')
                    throw newError(`Cannot transfer file: ${await response.text()}`, response.status);
                let json = null;
                try {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Old code before rule was applied
    async exec(method, path = '', query = null, body = null, headers = null, options = null) {
        for (let i = 0; i < 2; i++) {
            try {
