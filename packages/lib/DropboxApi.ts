...
export interface DropboxExecOptions {
	target?: 'string' | 'file';
	source?: 'string' | 'file';
	path?: string;
...
class DropboxApi {
	...
	private options_: DropboxOptions;
	private authToken_: string | null;
	private dispatcher_: EventDispatcher<string, any>;

	public constructor(options: DropboxOptions) {
	...
	public setAuthToken(v: string | null) {
		this.authToken_ = v;
	...
	public baseUrl(endPointFormat: 'content' | 'api') {
	...
	public async execAuthToken(authCode: string) {
		const postData: any = {
			code: authCode,
	...
	private isTokenError(status: number, responseText: string) {
	...
	public async exec(method: string, path = '', body: any = null, headers: any = null, options: DropboxExecOptions = null) {
		...
		while (true) {
			try {
				...
				tryCount++;
				if (error && typeof (error as any).code === 'string' && (error as any).code.indexOf('too_many_write_operations') >= 0) {
