...
export default class HtmlToMd {
	...
	public parse(html: string|HTMLElement, options: ParseOptions = {}) {
		const turndownOpts: Record<string, unknown> = {
			headingStyle: 'atx',
		...
		if (options.convertEmbeddedPdfsToLinks) {
			...
			turndownOpts.blankReplacement = (content: string, node: HTMLElement & { isCode?: boolean }) => {
		...
		const pdfRule = {
			filter: ['embed', 'object'],
			replacement: function(_content: string, node: HTMLElement, _options: Record<string, unknown>) {
