...
export default class Note extends BaseItem {
	...
	public static minimalSerializeForDisplay(note: NoteEntity) {
		...
		if (!n.is_locally_encrypted) pull(fieldNames, 'is_locally_encrypted');
	...
	public static previewFields(options: { includeTimestamps?: boolean } = null) {
		...
		const output = ['id', 'title', 'is_todo', 'todo_completed', 'todo_due', 'parent_id', 'encryption_applied', 'is_locally_encrypted', 'order', 'markup_language', 'is_conflict', 'is_shared', 'share_id', 'deleted_time'];
