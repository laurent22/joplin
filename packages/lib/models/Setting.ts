	...
	static metadata(): SettingItems {
		...
		this.metadata_ = {
			...
			'editor.mobile.removeSpaceBelowToolbar': {
				...
				label: () => _('Remove extra space below the markdown toolbar'),
				description: () => _('Works around bug on some devices where the markdown toolbar does not touch the bottom of the screen.'),
				storage: SettingStorage.File,
				isGlobal: true,
			},
			...
			useCustomPdfViewer: {
				...
				advanced: true,
				appTypes: [AppType.Desktop],
				label: () => _('Use custom PDF viewer (Beta)'),
				description: () => _('The custom PDF viewer remembers the last page that was viewed, however it has some technical issues.'),
				storage: SettingStorage.File,
				isGlobal: true,
			},
