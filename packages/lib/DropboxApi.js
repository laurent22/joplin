...
class DropboxApi {
    constructor(options) {
        ...
        this.dispatcher_ = new EventDispatcher_1.default();
    }
    clientId() {
    ...
    // private _requestToCurl_(url: string, options: any) {
    // 	const output = [];
    // 	output.push('curl');
    ...
    async execAuthToken(authCode) {
        ...
        const response = await shim_1.default.fetch('https://api.dropboxapi.com/oauth2/token', {
            method: 'POST',
            headers: {
    ...
    isTokenError(status, responseText) {
        if (status === 401)
            return true;
        if (responseText.indexOf('OAuth 2 access token is malformed') >= 0)
            return true;
        ...
        if (responseText.indexOf('Must provide HTTP header "Authorization"') >= 0)
            return true;
        return false;
    }
        ...
        while (true) {
            try {
                ...
                const newError = (message) => {
                    ...
                    const error = new JoplinError_1.default(`${method} ${path}: ${message} (${response.status}): ${shortResponseText}`, code);
                ...
                if (error && typeof error.code === 'string' && error.code.indexOf('too_many_write_operations') >= 0) {
                    ...
                    await time_1.default.sleep(tryCount * 2);
                }
                else {
                    throw error;
                }
...
//# sourceMappingURL=DropboxApi.js.map