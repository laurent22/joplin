...
interface TcpPortUsed {
	check(port: number): Promise<boolean>;
}

export async function findAvailablePort(tcpPortUsed: TcpPortUsed, possiblePorts: number[], extraRandomPortsToTry = 20) {
...
export function mimeTypeFromHeaders(headers: Record<string, string>) {
