...
export default class JoplinServerApi {
	...
	private async exec_(method: string, path = '', query: Record<string, any> = null, body: any = null, headers: any = null, options: ExecOptions = null) {
		...
		try {
			...
			if (!response.ok) {
				if (options.target === 'file') throw newError('Cannot transfer file', response.status);
				...
				// proxy. So in that case we keep the response content in error
				// details for logging and diagnostics, but keep the user-facing
				// message concise.
				//
				// It might be for example an error returned by the Nginx or
				// Apache reverse proxy. For example:
				//
				// <html>
				...
				//     </body>
				// </html>
				throw newError(`Error ${response.status} ${getHttpStatusMessage(response.status)}`, response.status);
			}
