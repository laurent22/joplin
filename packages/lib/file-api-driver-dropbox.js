...
class FileApiDriverDropbox {
    ...
    async get(path, options) {
        ...
        try {
            ...
            const needsFetchWorkaround = shim_1.default.mobilePlatform() === 'ios';
            const fetchPath = (method, path, extraHeaders) => {
                return this.api().exec(method, 'files/download', null, Object.assign({ 'Dropbox-API-Arg': JSON.stringify({ path: this.makePath_(path) }) }, extraHeaders), options);
            };
            let response;
            ...
            if (this.hasErrorCode_(error, 'not_found')) {
                ...
                throw new JoplinError_1.default('Cannot download because content is restricted by Dropbox', 'rejectedByTarget');
            }
            else {
                throw error;
            }
    ...
    async put(path, content, options = null) {
        try {
            ...
            if (this.hasErrorCode_(error, 'restricted_content')) {
                throw new JoplinError_1.default('Cannot upload because content is restricted by Dropbox (restricted_content)', 'rejectedByTarget');
            }
            else if (this.hasErrorCode_(error, 'payload_too_large')) {
                throw new JoplinError_1.default('Cannot upload because payload size is rejected by Dropbox (payload_too_large)', 'rejectedByTarget');
            }
            else {
                throw error;
            }
...
exports.default = FileApiDriverDropbox;
//# sourceMappingURL=file-api-driver-dropbox.js.map