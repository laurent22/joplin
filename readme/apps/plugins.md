...
Within the Joplin Plugins page you can hit the 'Delete' button on a plugin and it will be removed from the list. On desktop, Joplin must be restarted for this to complete. You may need to check Joplin is not minimising to the system tray/notification area rather than fully closing.
...
Plugin support on mobile is planned for Joplin v3.0 and is still in beta. As on desktop, plugins can be installed and managed from the configuration screen. Most plugins were originally developed for desktop, however, and will not work perfectly on mobile.
