...
Additional options are available in the `Note History` page of the Settings screen (*Windows/Linux*: Tools > Options > Plugins; *macOS*: Joplin > Preferences). It is possible to enable/disable the note history feature. It is also possible to specify for how long the history of a note should be kept (by default, for 90 days).
