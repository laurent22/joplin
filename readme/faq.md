...
Open Sublime (subl) in a new window (-n) and wait for it to close (-w):

```bash
subl -n -w
```

Open Visual Studio Code (code) in a new window (-n) and wait for it to close (--wait):

```bash
code -n --wait
```

Open gedit (Gnome Text Editor) in a new window:

```bash
gedit --new-window
```

Open a new terminal, then open vim:

```bash
xterm -e vim
```

Note: xterm can be replaced with an alternate terminal (gnome-terminal, terminator, etc.) and terminal text-editor (emacs, nano, etc.) of your choosing. 



**Mac only** - open a GUI application:
